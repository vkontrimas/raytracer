...
TEST_CASE("Camera initialization", "[camera][init]") {
    SECTION("Position and direction") {
        ...
        REQUIRE(camera.forward() == Vec3(0.0f, 0.0f, -1.0f));
...
TEST_CASE("Camera::lookAt()", "[camera][math]") {
    SECTION("At origin") {
        ...
        SECTION("X") {
            ...
            INFO(camera.forward());
            REQUIRE(camera.forward().approx(Vec3(1.0f, 0.0f, 0.0f)));
        }
        SECTION("-X") {
            camera.lookAt(Vec3(-10.0f, 0.0f, 0.0f));
            INFO(camera.forward());
        ...
        SECTION("Y") {
            ...
            INFO(camera.forward());
            REQUIRE(camera.forward().approx(Vec3(0.0f, 1.0f, 0.0f)));
        }
        SECTION("-Y") {
            camera.lookAt(Vec3(0.0f, -10.0f, 0.0f));
            INFO(camera.forward());
        ...
        SECTION("Z") {
            ...
            INFO(camera.forward());
            REQUIRE(camera.forward().approx(Vec3(0.0f, 0.0f, 1.0f)));
        }
        SECTION("-Z") {
            camera.lookAt(Vec3(0.0f, 0.0f, -10.0f));
            INFO(camera.forward());
        ...
        SECTION("Interesting vector") {
            ...
            INFO(camera.forward());
            REQUIRE(camera.forward().approx(Vec3(0.4447f, 0.3706f, 0.8153f)));
        }
    }
    ...
    SECTION("Away from origin") {
        ...
        INFO(camera.forward());
        REQUIRE(camera.forward().approx(Vec3(-0.4089f, 0.5842f, 0.7010f)));
    }
}
